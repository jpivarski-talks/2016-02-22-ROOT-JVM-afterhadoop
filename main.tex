...
Most importantly, I've discovered capabilities and constraints.
\end{block}

\begin{itemize}
\item Built clean Scala interface to TTrees using compile-time macros (next page).
...
{\it However,}
\begin{itemize}
\item Hadoop mappers had to copy the file from HDFS to local disk before reading.
\item Spark could only use my Hadoop InputFormat if it was precompiled in a JAR, not on the commandline (which dramatically changes Spark user experience). This is fixable.
