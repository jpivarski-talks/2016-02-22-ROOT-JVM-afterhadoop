...
// add "rootRDD" method to Spark's SparkContext
import org.dianahep.scaroot.spark._

val inputFormat = sc.rootRDD[Dimuon](
  "TrackResonanceNtuple*.root", "TrackResonanceNtuple/twoMuon")
\end{lstlisting}
\end{minipage}

\begin{itemize}
\item I'm internally passing my {\tt RootInputFormat} to Spark's {\tt sc.newAPIHadoopRDD}, but unfortunately this requires {\tt RootInputFormat} to have a zero-argument constructor. Scala inserts a hidden constructor argument to pass data to my macro, to overcome the JVM's type erasure. Workaround: custom RDD class.
\item Also, Hadoop's serialization is ignored by Spark; need to enable Kryo.
\end{itemize}
\end{frame}
